/**
 * GatewayModule.cpp - ESP32 Gateway (plugged into Mac via USB)
 *
...
// --- CẤU HÌNH ---
// MAC CỦA CON XE (Rover ESP32-S3): E8:F6:0A:83:92:08
...
// --- CALLBACK NHẬN DỮ LIỆU TỪ ROVER (Telemetry) ---
...
void setup() {
  ...
  Serial.println("\n=== ESP32 GATEWAY - Initializing ===\n");

  // 1. Kết nối WiFi để ĐỒNG BỘ KÊNH với Rover
  // 🔴 QUAN TRỌNG: Nếu bỏ bước này, Gateway ở kênh 1, Rover ở kênh khác -> MẤT
  // KẾT NỐI!
  WiFi.mode(WIFI_STA);
  ...
  // 2. In MAC address để cấu hình bên Rover
  ...
  Serial.printf("   Rover MAC: %02X:%02X:%02X:%02X:%02X:%02X\n", roverMAC[0],
                roverMAC[1], roverMAC[2], roverMAC[3], roverMAC[4],
                roverMAC[5]);
