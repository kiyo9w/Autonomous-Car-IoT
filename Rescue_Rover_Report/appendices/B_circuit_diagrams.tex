...
\subsection{L298N Wiring Notes}

\begin{infobox}[Important Wiring Notes]
\begin{enumerate}
    \item \textbf{Enable Jumpers:} Keep ENA and ENB jumpers installed for full-speed operation, or remove for PWM speed control.
    
    \item \textbf{Common Ground:} Ensure ESP32-S3 GND is connected to L298N GND.
    
    \item \textbf{Motor Polarity:} If motor spins in wrong direction, swap the two motor leads.
    
    \item \textbf{Heat Sink:} L298N requires heatsink for currents above 1A.
\end{enumerate}
\end{infobox}
...
\begin{warningbox}[5V to 3.3V Level Shifting]
The HC-SR04 ECHO pin outputs 5V, which may damage ESP32-S3 GPIO pins. Use a voltage divider or level shifter.
\end{warningbox}
