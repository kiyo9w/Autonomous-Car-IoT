...
Testing followed a structured approach that progressed from component isolation to full system integration. Each test category used specific metrics and measurement procedures documented in this section.
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{4cm}FIGURE: Test Environment Photograph\\Shows the room layout with obstacle course marked\vspace{4cm}}}
...
Communication tests measured the end to end latency and reliability of both the control channel (ESP-NOW) and the video channel (UDP streaming).
...
\begin{figure}[h!]
    \centering
    % \includegraphics[width=0.8\textwidth]{figures/charts/latency_histogram.png}
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{3.5cm}FIGURE: Command Latency Distribution\\Histogram with mean and P95 lines marked\vspace{3.5cm}}}
...
\begin{figure}[h!]
    \centering
    % \includegraphics[width=0.9\textwidth]{figures/charts/udp_vs_http_latency.png}
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{3.5cm}FIGURE: UDP vs HTTP Latency Comparison\\Box plot showing distribution for both protocols\vspace{3.5cm}}}
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.7\textwidth}{\centering\vspace{3cm}FIGURE: Packet Loss vs Distance\\Line graph showing degradation with range\vspace{3cm}}}
    \caption{Packet loss rate as a function of distance from the access point.}
    \label{fig:packet-loss}
\end{figure}

Within the 15 meter range typical for indoor operation, packet loss remains negligible. Beyond 20 meters, loss becomes noticeable but the system continues to function acceptably.
...
Low light conditions reduce frame rate because the camera increases exposure time. Motion during long exposures also causes blur, which increases JPEG size and occasionally causes frame drops.

% FIGURE PLACEHOLDER
\begin{figure}[h!]
    \centering
    % \includegraphics[width=0.8\textwidth]{figures/charts/fps_over_time.png}
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{3cm}FIGURE: FPS Over Time\\Time series showing frame rate during 5 minute test\vspace{3cm}}}
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{\textwidth}{\centering\vspace{4cm}FIGURE: JPEG Quality Comparison\\Grid of same scene at quality 10, 30, and 50\vspace{4cm}}}
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.7\textwidth}{\centering\vspace{3cm}FIGURE: Path Deviation Test\\Overhead view showing actual vs intended path\vspace{3cm}}}
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.6\textwidth}{\centering\vspace{3cm}FIGURE: Turning Trace\\Overhead path trace during 360 degree rotation\vspace{3cm}}}
...
YOLOv8 detection accuracy was evaluated using a test dataset of manually labeled frames captured from the rover camera.
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{3.5cm}FIGURE: Precision-Recall Curves\\Separate curves for each class\vspace{3.5cm}}}
    \caption{Precision-recall curves for each object class.}
    \label{fig:pr-curves}
\end{figure}

Person detection achieves the highest accuracy (mAP 0.89) because COCO training data contains many person examples. Door detection is weakest (mAP 0.68) likely due to variation in door appearance and partial occlusion.
...
The 12ms mean inference time is fast enough to process every frame at 30 FPS with significant headroom. Running on CPU-only hardware would increase this to approximately 45ms, still acceptable for operation.

% FIGURE PLACEHOLDER
\begin{figure}[h!]
    \centering
    % \includegraphics[width=0.7\textwidth]{figures/charts/inference_time_histogram.png}
    \fbox{\parbox{0.7\textwidth}{\centering\vspace{3cm}FIGURE: Inference Time Distribution\\Histogram of timing measurements\vspace{3cm}}}
...
The ultrasonic obstacle detection system was tested to verify it prevents collisions.
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.7\textwidth}{\centering\vspace{3cm}FIGURE: Ultrasonic Accuracy Plot\\Measured vs true distance with error bars\vspace{3cm}}}
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{3.5cm}FIGURE: Battery Discharge Curve\\Voltage over time during runtime test\vspace{3.5cm}}}
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.6\textwidth}{\centering\vspace{3cm}FIGURE: Thermal Camera Image\\Shows heat distribution across rover components\vspace{3cm}}}
...
\textbf{Root Cause:} The vLLM v1 engine (enabled by default in recent versions) exhibited instability when serving large multimodal models. Additionally, the default KV-cache settings consumed excessive VRAM, triggering out-of-memory conditions.
