...
\begin{figure}[h!]
    ...
    \definecolor{httpRed}{HTML}{D9534F}
    \definecolor{udpGreen}{HTML}{5CB85C}
    \definecolor{gridGray}{HTML}{E5E5E5}

    \begin{tikzpicture}[font=\sffamily\footnotesize, scale=1.0]
        % --- Setup Coordinates (Scale: Y=1ms -> 0.025cm) ---
        % HTTP: 200ms avg, range 180-220
        % UDP: 120ms avg, range 100-140
        
        % Grid and Axis
        \draw[draw=gridGray, thin] (0,0) grid[xstep=8cm, ystep=1.25] (7, 6.25); % Grid lines every 50ms
        
        % Y-Axis Labels
        \foreach \y/\label in {0/0, 1.25/50, 2.5/100, 3.75/150, 5.0/200, 6.25/250} {
            \node[anchor=east, text=gray] at (-0.1, \y) {\label};
        }
        \node[rotate=90, anchor=center] at (-1.0, 3.125) {End-to-End Latency (ms)};

        % X-Axis Line
        \draw[thick] (0,0) -- (7,0);

        % --- BAR 1: HTTP MJPEG ---
        % Position x=2, Height=200ms (5.0cm)
        \fill[fill=httpRed] (1.25, 0) rectangle (2.75, 5.0);
        \node[below] at (2, 0) {HTTP MJPEG};
        \node[white, font=\bfseries] at (2, 4.5) {200ms};
        
        % Error Bar (180ms to 220ms -> 4.5cm to 5.5cm)
        \draw[|-|, thick] (2, 4.5) -- (2, 5.5);
        
        % --- BAR 2: UDP Direct ---
        % Position x=5, Height=120ms (3.0cm)
        \fill[fill=udpGreen] (4.25, 0) rectangle (5.75, 3.0);
        \node[below] at (5, 0) {UDP Direct};
        \node[white, font=\bfseries] at (5, 2.5) {120ms};
        
        % Error Bar (100ms to 140ms -> 2.5cm to 3.5cm)
        \draw[|-|, thick] (5, 2.5) -- (5, 3.5);

        % --- ANNOTATIONS ---
        % Reference line from HTTP top
        \draw[dashed, gray] (2, 5.0) -- (5, 5.0);
        
        % Delta arrow (from 200ms down to 120ms at x=5)
        \draw[<->, >=stealth, thick, black] (5, 4.9) -- (5, 3.6) 
            node[midway, right, align=left, font=\scriptsize\bfseries, xshift=2pt] {$\Delta \approx 80$ms\\Reduction};

    \end{tikzpicture}
    \caption{Measured latency comparison. Error bars indicate the jitter range (min/max) observed during testing. UDP mode provides a consistent $\approx$40\% reduction in end-to-end latency compared to the HTTP MJPEG stream.}
