...
This chapter presents the overall system architecture of the Rescue Rover. The design follows a distributed intelligence model spread across three physical tiers and one cloud tier. This hybrid approach allows high-performance AI inference without compromising local real-time control.
...
\begin{enumerate}
    \item \textbf{The Rover (Edge Tier 1)}: ESP32-S3 microcontroller. Handles motor control, sensor readings, and video capture. It has no AI capability but provides 10ms-response safety reflexes.
    ...
    \item \textbf{The Cloud Server (Cloud Tier)}: Google Colab via Google Cloud Platform. Runs the "Strategic" AI layer (Qwen2.5-VL-7B) on NVIDIA A100 GPUs. It handles complex scene reasoning that is too heavy for the local host.
\end{enumerate}
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{\textwidth}{\centering\vspace{5cm}FIGURE: Hybrid Cloud System Overview\\Shows Rover, Gateway, Host (Mac), and Cloud (Colab) with links\vspace{5cm}}}
    \caption{High level system architecture showing the four processing nodes and their data links.}
    \label{fig:system-overview}
\end{figure}

\subsection{Device Specifications Summary}
...
\begin{table}[h!]
    ...
    \begin{tabular}{llp{6cm}}
        ...
        Cloud   & NVIDIA A100 GPU     & Strategic AI (Vision Language Model) \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[h!]
    ...
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{4cm}FIGURE: Hybrid Intelligence Pyramid\\Layer 1: Firmware, Layer 2: Local AI, Layer 3: Cloud AI\vspace{4cm}}}
    \caption{The three-layer hybrid intelligence model distributed across Edge and Cloud.}
    \label{fig:hybrid-layers}
\end{figure}

\subsection{Layer 1: Reactive Control (Firmware Level)}
The reactive layer runs on the ESP32-S3 firmware. It handles immediate threat preservation.
\begin{itemize}
...
\subsection{Layer 2: Tactical Processing (Local Host)}
The tactical layer runs on the local Mac using YOLOv8 (CoreML). It handles dynamic obstacles.
\begin{itemize}
...
\subsection{Layer 3: Strategic Planning (Cloud A100)}
The strategic layer runs on Google Colab using Qwen2.5-VL-7B. It handles complex navigation logic.
\begin{itemize}
...
\begin{table}[h!]
    ...
    \begin{tabular}{llll}
        ...
        \midrule
        Rover $\leftrightarrow$ Gateway & ESP-NOW & 2-5 ms & Commands, Telemetry \\
